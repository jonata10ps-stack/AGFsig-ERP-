...
// Utilitário para formatar moeda brasileira (Ex: 10.950,00)
const formatBRL = (val) => {
...
// Inteligência de Parsing: entende vários formatos e converte para número
const parseCurrency = (val) => {
  ...
  // Se não tem vírgula mas tem mais de um ponto, remove os pontos (1.250.000)
  else if ((clean.match(/\./g) || []).length > 1) {
    clean = clean.replace(/\./g, '');
  }
  // Se tem apenas um ponto e parece ser separador de milhar (ex: 1.250)
  // ou se o usuário digitou 1250.00 (padrão americano)
...
export default function PriceList() {
  ...
  const { data: products, isLoading } = useQuery({
    queryKey: ['products-price-list-smart'],
    queryFn: async () => {
  ...
  const updatePriceMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast.success('Valores processados e salvos!');
      setIsAdding(false);
      setProductSearch('');
    },
  });
  ...
  const handlePriceBlur = (productId, field, value) => {
    ...
    // Atualiza o estado de edição com o valor LIMPO (número)
    ...
    // Auto-formata o valor no input para o usuário ver o padrão
    const input = document.getElementById(`input-${productId}-${field}`);
    if (input) {
      input.value = formatBRL(numericValue);
    }
  };
    ...
    <div className="space-y-6 max-w-[1400px] mx-auto p-4 lg:p-0">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        <div>
          ...
          <p className="text-slate-500 font-medium tracking-tight">Os valores são auto-formatados no padrão 10.950,00 ao sair do campo.</p>
        </div>
        
        {canEdit && (
          <Button 
            onClick={() => setIsAdding(!isAdding)}
            className="bg-indigo-600 hover:bg-indigo-700 shadow-xl font-bold rounded-xl text-white h-12 px-6"
          >
          ...
          <CardContent className="p-8">
            <div className="relative">
              ...
              <Input
                ...
                className="pl-14 h-16 bg-slate-50 border-none rounded-xl text-xl font-medium"
              />
            </div>
              ...
              <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                {searchResults.map(p => (
                  <div key={p.id} className="flex items-center justify-between p-5 bg-white rounded-2xl border border-slate-100 hover:border-indigo-200 hover:bg-indigo-50/30 transition-all group cursor-pointer" onClick={() => saveProductPrice(p, { monoVista: 0.01 })}>
                    ...
                    <Button size="sm" className="bg-indigo-600 ml-4 h-10 px-4 text-white font-bold rounded-lg shrink-0">
                      Selecionar
                    </Button>
                  </div>
                ))}
        ...
        <CardHeader className="bg-slate-50/50 border-b border-slate-100 py-6 px-8">
          <div className="relative max-w-md">
            ...
            <Input
              ...
              className="pl-12 h-12 bg-white border-none rounded-xl shadow-sm text-sm"
            />
          </div>
        </CardHeader>
                ...
                {isLoading ? (
                  <tr>
                    <td colSpan={canEdit ? 6 : 5} className="py-32 text-center">
                      ...
                      <span className="text-slate-400 font-bold tracking-widest">SINCRONIZANDO...</span>
                    </td>
                  </tr>
                    ...
                    <tr key={product.id} className="group hover:bg-slate-50 transition-all duration-150">
                      ...
                      {/* Inputs agrupados com IDs para auto-formatação no Blur */}
                      <td className="px-4 py-6 bg-indigo-50/5">
                        <div className="relative w-36 mx-auto">
                          <span className="absolute left-3 top-1/2 -translate-y-1/2 text-[10px] font-black text-indigo-300">R$</span>
                          <Input
                            id={`input-${product.id}-monoVista`}
                            readOnly={!canEdit}
                            className={`pl-10 h-10 font-black text-right pr-3 ${!canEdit ? 'bg-transparent border-transparent shadow-none' : 'bg-white border-slate-200 rounded-xl'}`}
                            defaultValue={formatBRL(product.sale_price)}
                            onBlur={(e) => handlePriceBlur(product.id, 'monoVista', e.target.value)}
                          />
                        </div>
                      </td>
                      <td className="px-4 py-6 bg-indigo-50/5 border-r border-slate-100">
                        <div className="relative w-36 mx-auto">
                          <span className="absolute left-3 top-1/2 -translate-y-1/2 text-[10px] font-black text-indigo-300">R$</span>
                          <Input
                            id={`input-${product.id}-monoPrazo`}
                            readOnly={!canEdit}
                            className={`pl-10 h-10 font-black text-right pr-3 ${!canEdit ? 'bg-transparent border-transparent shadow-none' : 'bg-white border-slate-200 rounded-xl'}`}
                            defaultValue={formatBRL(product.unit_price)}
                            onBlur={(e) => handlePriceBlur(product.id, 'monoPrazo', e.target.value)}
                          />
                        </div>
                      </td>
                      ...
                      <td className="px-4 py-6 bg-amber-50/5">
                        <div className="relative w-36 mx-auto">
                          <span className="absolute left-3 top-1/2 -translate-y-1/2 text-[10px] font-black text-amber-300">R$</span>
                          <Input
                            id={`input-${product.id}-triVista`}
                            readOnly={!canEdit}
                            className={`pl-10 h-10 font-black text-right pr-3 ${!canEdit ? 'bg-transparent border-transparent shadow-none' : 'bg-white border-slate-200 rounded-xl'}`}
                            defaultValue={formatBRL(product.quantity)}
                            onBlur={(e) => handlePriceBlur(product.id, 'triVista', e.target.value)}
                          />
                        </div>
                      </td>
                      <td className="px-4 py-6 bg-amber-50/5 border-r border-slate-100">
                        <div className="relative w-36 mx-auto">
                          <span className="absolute left-3 top-1/2 -translate-y-1/2 text-[10px] font-black text-amber-300">R$</span>
                          <Input
                            id={`input-${product.id}-triPrazo`}
                            readOnly={!canEdit}
                            className={`pl-10 h-10 font-black text-right pr-3 ${!canEdit ? 'bg-transparent border-transparent shadow-none' : 'bg-white border-slate-200 rounded-xl'}`}
                            defaultValue={formatBRL(product.min_stock)}
                            onBlur={(e) => handlePriceBlur(product.id, 'triPrazo', e.target.value)}
                          />
                        </div>
                      </td>
                      ...
                      {canEdit && (
                        <td className="px-8 py-6 text-right">
                          <Button
                            size="sm"
                            className="bg-emerald-600 hover:bg-emerald-700 shadow-md text-white font-black uppercase text-[10px] tracking-widest rounded-xl transition-all opacity-0 group-hover:opacity-100"
