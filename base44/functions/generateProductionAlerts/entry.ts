...
Deno.serve(async (req) => {
  ...
  try {
    ...
    for (const step of steps) {
      ...
      if (endDate < today) {
        // Overdue step
        const daysLate = Math.round(((today as any) - (endDate as any)) / (1000 * 60 * 60 * 24));
        alerts.push({
          type: 'ETAPA_ATRASADA',
        ...
        // About to be late
        const daysLeft = Math.round(((endDate as any) - (today as any)) / (1000 * 60 * 60 * 24));
        alerts.push({
    ...
    for (const op of orders) {
      ...
      if (op.due_date && op.status !== 'ENCERRADA') {
        ...
        if (due < today) {
          const daysLate = Math.round(((today as any) - (due as any)) / (1000 * 60 * 60 * 24));
          alerts.push({
            type: 'OP_STATUS_CRITICA',
    ...
    return new Response(JSON.stringify({ alerts: alerts.slice(0, 30) }), {
      headers: { ...corsHeaders, 'Content-Type': 'application/json' }
    });
  } catch (error) {
